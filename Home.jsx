    ...
    {options ? (
      <div className="relative">
        <select
          ...
          value={value}
          onChange={onChange}
          className="w-full bg-slate-50 border border-slate-200 text-slate-800 p-4 font-medium text-base focus:outline-none focus:bg-white focus:border-slate-900 focus:ring-1 focus:ring-slate-900 transition-all appearance-none rounded-xl cursor-pointer"
        >
      ...
      <input
        ...
        className="w-full bg-slate-50 border border-slate-200 text-slate-800 p-4 font-medium text-base focus:outline-none focus:bg-white focus:border-slate-900 focus:ring-1 focus:ring-slate-900 transition-all placeholder:text-slate-400 rounded-xl"
      />
    )}
  </div>
);
...
const BookingForm = ({ formData, handleInputChange, handleSubmit, isEdit, isFull, usersInCurrentSlot }) => {
    return (
        <form onSubmit={handleSubmit} className="w-full">
            ...
            {/* Status Box */}
            <div className={`mt-4 mb-6 rounded-2xl p-5 md:p-6 transition-all duration-300 border ${
                isFull 
...
export default function GymBookingApp() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.name || !formData.date || !formData.slot) return;
